...
class AgentTaskState(BaseModel):
    ...
    job_id: str = Field(
        ...,
        description="The unique identifier for the specific job/task."
    )

    owner_id: str = Field(
        ...,
        description=(
            "The owner_id inherited from the authenticated user, "
    ...
    structured_state: Dict[str, Any] = Field(
        default_factory=dict,
        description="Compact structured state accumulated by the agent."
    )

    evidence_references: List[str] = Field(
        default_factory=list,
        description=(
            "List of IDs pointing to existing structured evidence."
        )
    )

    # Conflict policy metadata
    conflict_logs: List[Dict[str, Any]] = Field(
        default_factory=list,
        description="Logs documenting conflicts detected across sources."
    )
